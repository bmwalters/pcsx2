...
s32 USBopen(NativeWindowHandle* pGSWindowHandle)
{
	if (conf.Log && !usbLog)
	{
	...
	gsWindowHandle = pGSWindowHandle;

	try
	{
		shared::Initialize(pGSWindowHandle);
	}
	...
	{
		CreateDevices(); //TODO Pass pGSWindowHandle to init?
	}

	OpenDevice(0 /*, pGSWindowHandle */);
	OpenDevice(1 /*, pGSWindowHandle */);
	usb_opened = true;
	return 0;
}
